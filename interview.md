...
티디디와 비디디는 방법에 있어서는 큰 차이는 없다고 생각한다. 우선 티디디는 개발자가 받은 기획서에 맞게 개발에 요구되는 기능을 작은 단위부터 테스트코드부터 만든 뒤 테스트 코드를 통과 할수 있게 코드를 최적화 하면서 개발하는 방식이고, 비디디는 이런 테스트 단위를 좀 더 스토리(에자일 방법론)에 맞게 예를 들어 라면을 만드는 프로그램을 짠다고 가정하면 라면이 있는지 확인한다. 라면을 뜬는다. 라면을 뜯어 스프와 면을 분리한다. 냄비에 물을 붙는다. 냄비를 가스렌지위에 올려논는다.... 등등 스로리에 맞게 개발하는 방법론이다. 우리가 만약 함수형으로 작업을 한다면 비디디에 맞게 개발하는게 자연스러워 진다. 또한 에자일 방법론에 맞게 티켓을 관리 할 수 있다. 

### 안드로이드, IOS 에 대한 테스팅 방법 및 다르게 구현해야 하는 이슈 경험 사례
안드로이드와 ios의 내장 브라우저가 다르고 디바이스 동작 방식이 다르기 때문에 테스트가 디바이스 환경에 따라 달라질수 밖에 없다.

### 주소를 칠때부터 사용자에게 보여질때까지 웹페이지가 렌더링 되는 과정 설명

우리가 브라우저에서 특정 웹서버에 html 문서를 요청하여 받아 오는 프로토콜은 http(https)이다. 주소를 치면 그 주소에 웹서버에 http 프로토콜로 요청한다. 

과정으로 설명하면
0. 핸드쉐이크 클라 서버간 연결 준비완료 TCP 세션 수립 (연결 수립)
1. 특정 주소의 index.html 요청
2. 응답을 돌려준다. html > css > js (패킷 구조로 전송 (header 와 body로 구성됨))
3. 응답후 서버는 연결 종료 대기 상태를 클라이언트에 고지
4. 클라이언트의 승인 (여기서 만약 다시 요청한다면 연결 상태를 다시 만들지 않기 때문에 좀 더 빨리 받을 수 있는것이다.)
5. 서버는 연결 종료 

일단 적인 네트워크 과정은 이러하고 브라우저에서 응답온 패킷을 해석하는 과정은 렌러딩이라는 말로 대체할 수 있다. 
브라우저 마다 이런 렌더링 엔진이 있는데 구글을 많이 쓰기에 웹킷 엔진에서의 과정을 설명한다.

1. html 파일 파싱하여 dom 트리를 생성
2. css 해석 후 스타일 규칙 생성 CSSOM 트리
3. dom 트리와 스타일 규칙이 합성 이과정에서 화면에 노출하지 않아도 되는 항목 head 태그 display none등은 렌더링 트리에 포함 시키지 않음.
4. 렌더링 트리 생성 후 스타일 규칙에 의거한 레이아웃 과정. (이과정에서 비율로 정의된 값들이 절대값으로 변환)
4. 적용된 렌더링 트리를 그리고 화면에 표시한다.

여기에서 알수 있는건 우리가 스크립트를 body에 넣는 이유를 알수  있다. 우리가 head에 스크립트를 넣으면 파서가 head읽은 위치에서 스크립트 파일을 해석 하므로 body 렌더링 트리를 그리는 시간이 촉박하다. 물론 이방법은 자바스크립트로 화면을 제어한다면 화면이 잠깐 깜빡이는 현상이 이뤄날수 있지만 이건 또다른 해결 방법이 존재한다.


### HTTP 헤더값 중 connection의 keep-alive 속성 설명
http 통신 과정에서 http 통신은 연결과 해지가 반복적으로 이뤄진다. 만약 사용자가 여러가지 요청을 연속적으로 해야 할 상활이라면 keep-alive 속성을 넣어서 연결을 이어지게 해주고 마지막 요청에서 close 속성을 주어 연결을 끊어지게 해주는것이 요청 속도를 좀더 빠르게 확보 할수 있는 방법이다.

### SSL에 대한 전반적인 설명

접속하는 사이트가 안전한지 않한지 인증기관을 거쳐 인증하게 되는 인증서

### RESTFul CRUD 메소드에 대한 및 설명 : GET, POST, PUT, DELETE

rest의 특징으로는 쿠키, 세션정보나 별도로 관리 하지 않는다. 비연결지향 연결이 요청후 응답후 끊어지게 기본적으로 되어 있다. 그리고 그 상태를 자체적으로 세션이나 쿠키로 관리하지 않고 http기능을 사용한다(쿠키, 세션).
갯은 조회의 역할이고 포스트는 생성의 역할 풋은 수정의 역할 딜릿은 삭제의 역할이다.

### HTTP 응답코드 200, 300, 400, 500번대별 설명

200번대는 정상 응답
300번대는 정상 응답을 위해 추가적은 조치가 필요하다는 뜻
400번대는 클라이언트 오류 일 경우다
500번대는 서버의 오류 일 경우다 

### 세션 쿠키 설명

쿠키의 특직은 만료날짜가 있다는 점.
최대 300개의 저장, 하나의 도메인엔 20개, 하나의 값은 4kb다.

세션은 쿠키와 다르게 서버에 정보가 저장된다. 그러므로 보안처리를 서버에서 해주기 때문에 더 안전
하지만 세션으로 모든 계정을 처리하면 서버에 과부화 발생.

쿠키의 생명은 만료날짜 지정순간 까지 가능하지만 세션은 접속에 끝나면 바로 사라진다.

### React의 state와 구 AngularJS의 2 Way 바인딩의 차이

엥귤러의 경우는 모델과 뷰가 서로 같은 스콥에서는 서로 변경에 대한 영향을 받고 데이터를 공유하는 것이다,
리액트는 상위에서 하위로 데이터를 흘려주고 상위로 이를 전달하기 위해서 다른 방법을 이용해야 한다.

### 점진적 향상과 우아한 낮춤 설명

기능을 하나하나 기초적인 기술 기반으로 추가하면서 충분한 검토를 거치면서 만들어 나간다.
우아한 낮춤은 일단 최신 기술등을 모두 쏟아 동작 되게 만든뒤 환경 맞게 점차적으로 기능에 대한 예외처리를 하는 방법이다. 
